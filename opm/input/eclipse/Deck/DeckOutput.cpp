...
namespace Opm {
    ...
    void DeckOutput::write( const T& value ) {
        ...
        write_token( format_value( value ) );
    }
    ...
    void DeckOutput::write_sep( size_t next_token_width ) {
        if (record_on && (row_count > 0)) {
            bool split = this->split_line && ((row_count % this->fmt.columns) == 0);

            // Split before the line exceeds the maximum width; reserve two characters
            // for the " /" record terminator.
            if (this->fmt.max_line_width > 0 &&
                this->current_width + this->fmt.item_sep.size() + next_token_width + 2 > this->fmt.max_line_width)
                split = true;
